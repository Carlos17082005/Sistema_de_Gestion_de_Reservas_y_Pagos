...
<?php
	...
	try {
		...
		try {
			...
			$id_reserva = id_reserva($conn);
			foreach($carrito as $id => $cantidad)  {
				comprar($conn, $id, $cantidad);
				registrar($conn, $id, $cantidad, $id_reserva);
			}

			echo '<p style="text-align: center; color: green;">Compra realizada con exito</p>';
			setcookie("id_reserva", '', time() - 3600, '/');
			$carrito = [];
