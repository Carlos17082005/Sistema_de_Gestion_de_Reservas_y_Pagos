<html>
   
 <head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta http-equiv="X-UA-Compatible" content="ie=edge">
    <?php include 'funciones/funciones.php';?>
    <title>Login Page - PORTAL RESERVAS</title>
    <link rel="stylesheet" href="./css/bootstrap.min.css">
 </head>
      
<body>
    

    <div class="container ">
        <!--Aplicacion-->
		<div class="card border-success mb-3" style="max-width: 30rem;">
		<div class="card-header">Acceso Reserva Vuelos</div>
		<div class="card-body">
		
		<form id="login" name="login" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="post" class="card-body">
		
		<div class="form-group">
			Usuario <input type="text" name="usuario" placeholder="usuario" class="form-control">
        </div>
		<div class="form-group">
			Password <input type="password" name="password" placeholder="password" class="form-control">
        </div>				
        
		<input type="submit" name="submit" value="Login" class="btn btn-warning disabled">
        </form>
		
	    </div>
    </div>
    </div>
    </div>
    <?php
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            $usuario = test_input($_POST['usuario']);
            $password = test_input($_POST['password']);

            try {
                $conn = conexionBD();

                list($dni, $nombre, $acceso) = login($conn, $usuario, $password);

                if ($acceso)  {
                    echo '<p style="text-align: center; color: green;">Login correcto</p>';
                    
                    session_start();
                    $_SESSION['email'] = $usuario;
                    $_SESSION['nombre'] = $nombre;
                    $_SESSION['dni'] = $dni;

                    $fecha = date("Y-m-d H-i-s");
                    setcookie("fecha", $fecha, time() + 3600, '/');

                    header("Location: vinicio.php");
                    exit;

                }  else  {
                    throw new Exception('<p style="text-align: center; color: red;">Login incorrecto</p>');
                }
                
            }  catch(PDOException $e)  {
                error($e);

            }  catch(Exception $e)  {
                echo $e->getMessage();

            }  finally  {
                if ($conn !== null) {
                    $conn = null;
                }
            }  
        }
    ?>
</body>
</html>